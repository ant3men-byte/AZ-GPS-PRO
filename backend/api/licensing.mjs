...
export default async function handler(req,res){
 ...
 if(!/^(license|admin\/licenses)(\/[-a-zA-Z0-9]+)*$/.test(route)){res.statusCode=404;res.end('Not found');return;}
 const params=new URLSearchParams();if(typeof req.query?.search==='string')params.set('search',req.query.search.slice(0,80));
 const headers=new Headers();for(const name of ['authorization','content-type','content-length']){const value=req.headers[name];if(typeof value==='string')headers.set(name,value);}
